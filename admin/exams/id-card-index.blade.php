        ...
        <div class="card border-0 shadow-sm rounded-3 w-100">
            ...
            <div class="card-body px-3 px-md-4 pt-3">
                <div class="d-flex align-items-center gap-2 mb-4 p-3 rounded-3" style="background: #f8fafc; border: 1px solid #eef2f7;">
                    ...
                    <div>
                        <h5 class="fw-bold mb-0" style="color: #1e293b;">{{ $exam->name }}</h5>
                        <div class="d-flex gap-3 flex-wrap small text-muted">
                            <span><i class="fas fa-book me-1"></i> {{ $exam->subject->name ?? '-' }}</span>
                            <span><i class="fas fa-calendar me-1"></i> {{ $exam->date ? $exam->date->format('d/m/Y') : '-' }}</span>
                            <span><i class="fas fa-clock me-1"></i> {{ $exam->start_time ? substr($exam->start_time, 0, 5) : '-' }} - {{ $exam->end_time ? substr($exam->end_time, 0, 5) : '-' }}</span>
                            <span><i class="fas fa-tag me-1"></i> {{ strtoupper($exam->type) }}</span>
                            <span><i class="fas fa-chalkboard-teacher me-1"></i> {{ $exam->teacher->name ?? '-' }}</span>
                        </div>
                    </div>
                </div>

                <form action="{{ route('admin.exams.id-card-print', $exam) }}" method="GET" id="printForm" target="_blank">
                    <div class="d-flex align-items-center justify-content-between mb-3 flex-wrap gap-2">
                        <div class="d-flex align-items-center gap-2">
                            <span class="badge bg-primary bg-opacity-10 text-primary rounded-pill px-3 py-2 fw-normal small">
                                <i class="fas fa-layer-group me-1"></i> {{ $students->count() }} siswa
                            </span>
                            ...
                            <span class="badge rounded-pill px-3 py-2 fw-normal" style="background: #eef2ff; color: #4f46e5; font-size: 0.75rem;">
                                <i class="fas fa-school me-1"></i> {{ $class->name }}
                            </span>
                            @endforeach
                        </div>
                    </div>
                            ...
                            <tbody>
                                ...
                                <tr style="border-bottom: 1px solid #f1f5f9; transition: all 0.2s;" onmouseover="this.style.background='#f8fafc'" onmouseout="this.style.background=''">
                                    <td class="text-center py-3">
                                        <div class="form-check">
                                            <input class="form-check-input student-check" type="checkbox" name="students[]" value="{{ $student->id }}">
                                        </div>
                                    </td>
                                    <td class="text-muted py-3" style="font-size: 0.8rem;">{{ $loop->iteration }}</td>
                                    <td class="py-3"><span class="badge bg-light text-muted font-monospace rounded-pill px-3 py-1.5 fw-normal" style="font-size: 0.75rem;">{{ $student->nis }}</span></td>
                                    <td class="py-3">
                                        <div class="d-flex align-items-center gap-2">
                                            ...
                                            <span class="fw-semibold" style="color: #1e293b; font-size: 0.9rem;">{{ $student->user->name }}</span>
                                        </div>
                                    </td>
                                    <td class="py-3">
                                        <span class="badge rounded-pill px-3 py-1.5 fw-normal" style="background: #eef2ff; color: #4f46e5; font-size: 0.75rem;">
                                            <i class="fas fa-school me-1"></i> {{ $student->class->name ?? '-' }}
                                        </span>
                                    </td>
                                </tr>
                                @empty
                    ...
                    <div class="d-flex justify-content-between align-items-center mt-3 flex-wrap gap-2">
                        ...
                        <div class="d-flex gap-2">
                            <a href="{{ route('admin.exams.index') }}" class="btn btn-outline-secondary rounded-pill px-3">
